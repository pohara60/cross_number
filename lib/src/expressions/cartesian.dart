int cartesianCount<T>(List<List<T>> inputs) {
  return inputs.fold(1, (previousValue, element) => previousValue * element.length);
}

Iterable<List<T>> cartesian<T>(List<List<T>> inputs, [bool duplicates = false]) sync* {
  if (inputs.isEmpty) {
    yield <T>[];
    return;
  }
...
Iterable<List<T>> cartesianDuplicateLimit<T>(List<List<T>> inputs, Map<int, int> duplicateLimit) sync* {
  if (inputs.isEmpty) {
    yield <T>[];
    return;
  }
  ...
  do {
    while (cursor < indices.length) {
      ...
      var count = result.isEmpty
          ? 0
          : result.map((element) => element == value ? 1 : 0).reduce((value, element) => value + element);
